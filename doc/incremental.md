...
Rio data values are compared based on their value, not identity.

In Rio, we use *intensional* equality to compare functions.  Two functions
are equal if they share the same internal structure *and* their captured
variables have equal values.

The details of structure comparison is left somewhat loose, but the key
requirements that must be met are: (1) intensional equality must imply
extensional equality ("when two functions are 'equal', they must behave the
same, but not necessarily vice versa"), and (2) if the program text has not
changed, then two instances of the same expression with the same captured
